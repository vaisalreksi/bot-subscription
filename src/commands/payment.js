...
export default {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            // Determine column based on username
            const column = username === 'cucudukun' ? 'G' : 'E';

            // Add payment entry to sheet
            const rowInserted = await googleSheetsService.addPaymentEntry(config.sheet, dateStr, column);
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '📍 Cell', value: `${column}${rowInserted}`, inline: true }
                )
                .setImage(proof.url)
                .setTimestamp()
                .setFooter({
                    text: `Recorded by ${username}`,
